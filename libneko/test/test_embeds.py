...
from neko2.libneko.embeds import *
        ...
        if isinstance(o, list):
            ...
            for k, v in o.items():
                self.assertIsInstance(
                    k, str, f"Expected key to be str in nested level {nest}"
                )
                self._assert_members_are_jsonifyable_recursive(v, nest + 1)
        else:
            none_type = type(None)
            if not isinstance(
                o, (none_type, int, float, str, bool, type(unspecified_field))
            ):
...
class EmbedURLTest(DictifyMixin, unittest.TestCase):
    ...
    def test_author_url(self):
        e = Embed().set_author(
            name="foobar", url="http://google.com"
        )  # Should be valid.
        self._dictify(e)
        with self.assertRaises(InvalidEmbedURL):
    ...
    def test_author_icon_url(self):
        e = Embed().set_author(
            name="foobar", icon_url="http://google.com"
        )  # Should be valid.
        self._dictify(e)
    ...
    def test_footer_icon_url(self):
        e = Embed().set_footer(
            text="foobar", icon_url="http://google.com"
        )  # Should be valid.
        self._dictify(e)
